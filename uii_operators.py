    ...
    def apply(self, env_signal: Dict, internal_state: 'CompressionOperator') -> 'SensingOperator':
        ...
        for cid, channel in self.channels.items():
            if not channel.active:
                # Dark channel: decay coverage
                new_coverage = channel.coverage * 0.9
                updated[cid] = dataclasses.replace(channel, coverage=new_coverage)
                continue
...
class SelfModifyingOperator:
    ...
    def _smo_s(self,
        ...
        Does NOT activate new channels.
        """
        updated_channels = {}
        total_delta      = 0.0
